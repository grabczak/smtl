...
testSmtlibVarExpr = TestCase $ do
  let prog = Program [Declare [makeLoc "x"] Int, Assert (makeLoc (Var "x"))]
...
testSmtlibAdd = TestCase $ do
  let prog = Program [Assert (makeLoc (Geq (makeLoc (Add (makeLoc (IntLit 2)) (makeLoc (IntLit 3)))) (makeLoc (IntLit 4))))]
  let output = smtlib prog
  assertBool "Addition in program" ("(+" `isInfixOf` output)

testSmtlibSub :: Test
testSmtlibSub = TestCase $ do
  let prog = Program [Assert (makeLoc (Geq (makeLoc (Sub (makeLoc (IntLit 10)) (makeLoc (IntLit 3)))) (makeLoc (IntLit 0))))]
  let output = smtlib prog
  assertBool "Subtraction in program" (length output > 0)

testSmtlibMul :: Test
testSmtlibMul = TestCase $ do
  let prog = Program [Assert (makeLoc (Geq (makeLoc (Mul (makeLoc (IntLit 2)) (makeLoc (IntLit 3)))) (makeLoc (IntLit 0))))]
  let output = smtlib prog
  assertBool "Multiplication in program" ("(*" `isInfixOf` output)

testSmtlibNeg :: Test
testSmtlibNeg = TestCase $ do
  let prog = Program [Assert (makeLoc (Geq (makeLoc (Neg (makeLoc (IntLit (-5))))) (makeLoc (IntLit 0))))]
  let output = smtlib prog
...
testSmtlibAnd = TestCase $ do
  let prog = Program [Assert (makeLoc (And (makeLoc (BoolLit True)) (makeLoc (BoolLit False))))]
  let output = smtlib prog
  assertBool "And in program" ("(and" `isInfixOf` output)

testSmtlibOr :: Test
testSmtlibOr = TestCase $ do
  let prog = Program [Assert (makeLoc (Or (makeLoc (BoolLit True)) (makeLoc (BoolLit False))))]
  let output = smtlib prog
  assertBool "Or in program" ("(or" `isInfixOf` output)

testSmtlibNot :: Test
testSmtlibNot = TestCase $ do
  let prog = Program [Assert (makeLoc (Not (makeLoc (BoolLit True))))]
  let output = smtlib prog
  assertBool "Not in program" ("(not" `isInfixOf` output)

testSmtlibImplies :: Test
testSmtlibImplies = TestCase $ do
  let prog = Program [Assert (makeLoc (Implies (makeLoc (BoolLit True)) (makeLoc (BoolLit False))))]
  let output = smtlib prog
  assertBool "Implies in program" ("(=>" `isInfixOf` output)

testSmtlibIff :: Test
testSmtlibIff = TestCase $ do
  let prog = Program [Assert (makeLoc (Iff (makeLoc (BoolLit True)) (makeLoc (BoolLit True))))]
  let output = smtlib prog
...
testSmtlibEq = TestCase $ do
  let prog = Program [Assert (makeLoc (Eq (makeLoc (IntLit 5)) (makeLoc (IntLit 5))))]
  let output = smtlib prog
  assertBool "Eq in program" ("=" `elem` words output || "5" `elem` words output)

testSmtlibNeq :: Test
testSmtlibNeq = TestCase $ do
  let prog = Program [Assert (makeLoc (Neq (makeLoc (IntLit 3)) (makeLoc (IntLit 4))))]
  let output = smtlib prog
  assertBool "Neq in program" ("(not" `isInfixOf` output)

testSmtlibLt :: Test
testSmtlibLt = TestCase $ do
  let prog = Program [Assert (makeLoc (Lt (makeLoc (IntLit 2)) (makeLoc (IntLit 5))))]
  let output = smtlib prog
  assertBool "Lt in program" ("(<" `isInfixOf` output)

testSmtlibGt :: Test
testSmtlibGt = TestCase $ do
  let prog = Program [Assert (makeLoc (Gt (makeLoc (IntLit 10)) (makeLoc (IntLit 3))))]
  let output = smtlib prog
  assertBool "Gt in program" ("(>" `isInfixOf` output)

testSmtlibLeq :: Test
testSmtlibLeq = TestCase $ do
  let prog = Program [Assert (makeLoc (Leq (makeLoc (IntLit 3)) (makeLoc (IntLit 8))))]
  let output = smtlib prog
  assertBool "Leq in program" ("(<=" `isInfixOf` output)

testSmtlibGeq :: Test
testSmtlibGeq = TestCase $ do
  let prog = Program [Assert (makeLoc (Geq (makeLoc (IntLit 15)) (makeLoc (IntLit 5))))]
  let output = smtlib prog
...
testSmtlibComplexArithmetic = TestCase $ do
  let prog = Program [Assert (makeLoc (Geq (makeLoc (Add (makeLoc (Mul (makeLoc (IntLit 2)) (makeLoc (IntLit 3)))) (makeLoc (IntLit 4)))) (makeLoc (IntLit 0))))]
  let output = smtlib prog
...
testSmtlibComplexComparison = TestCase $ do
  let prog = Program [Assert (makeLoc (Leq (makeLoc (Add (makeLoc (Mul (makeLoc (IntLit 2)) (makeLoc (Var "x")))) (makeLoc (IntLit 5)))) (makeLoc (IntLit 100))))]
  let output = smtlib prog
...
testSmtlibLogicDeclaration = TestCase $ do
  let prog = Program []
  let output = smtlib prog
  assertBool "SMT-LIB output starts with logic declaration" ("(set-logic QF_UFLIA)" `elem` lines output)
...
testSmtlibMultipleVariables = TestCase $ do
  let prog =
        Program
          [ Declare [makeLoc "x", makeLoc "y", makeLoc "z"] Int
          , Assert (makeLoc (Leq (makeLoc (Add (makeLoc (Var "x")) (makeLoc (Var "y")))) (makeLoc (Var "z"))))
          ]
  let output = smtlib prog
...
testSmtlibConstraint = TestCase $ do
  let prog =
        Program
          [ Declare [makeLoc "product_a", makeLoc "product_b"] Int
          , Assert (makeLoc (Leq (makeLoc (Add (makeLoc (Mul (makeLoc (IntLit 2)) (makeLoc (Var "product_a")))) (makeLoc (Mul (makeLoc (IntLit 3)) (makeLoc (Var "product_b")))))) (makeLoc (IntLit 40))))
          ]
  let output = smtlib prog
...
testSmtlibEmpty = TestCase $ do
  let prog = Program []
  let output = smtlib prog
  assertBool "Empty program generates valid skeleton" ("(set-logic QF_UFLIA)" `elem` lines output)
...
testSmtlibMultipleAssertions = TestCase $ do
  let prog =
        Program
          [ Declare [makeLoc "x"] Int
          , Assert (makeLoc (Geq (makeLoc (Var "x")) (makeLoc (IntLit 0))))
          , Assert (makeLoc (Leq (makeLoc (Var "x")) (makeLoc (IntLit 100))))
          ]
  let output = smtlib prog
...
testSmtlibAssignmentSubstitution = TestCase $ do
  let prog =
        Program
          ...
          , Assert (makeLoc (Eq (makeLoc (Var "x")) (makeLoc (IntLit 5))))
          ]
  let output = smtlib prog
...
testSmtlibComplexConstraintSystem = TestCase $ do
  let prog =
        Program
          [ Declare [makeLoc "a", makeLoc "b", makeLoc "c"] Int
          , Assert (makeLoc (And (makeLoc (Geq (makeLoc (Var "a")) (makeLoc (IntLit 0)))) (makeLoc (And (makeLoc (Leq (makeLoc (Var "b")) (makeLoc (IntLit 100)))) (makeLoc (Geq (makeLoc (Var "c")) (makeLoc (IntLit 0))))))))
          ]
  let output = smtlib prog
